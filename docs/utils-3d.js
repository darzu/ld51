...
export function randNormalVec3(out) {
    vec3.set(out, Math.random() - 0.5, Math.random() - 0.5, Math.random() - 0.5);
    vec3.normalize(out, out);
    return out;
}
export function randNormalVec2(out) {
...
export function quatFromUpForward(out, up, forwardish) {
...
export function farthestPointInDir(points, d) {
    let max = -Infinity;
    let maxP = null;
    for (let p of points) {
...
export function uintToVec3unorm(i, max) {
    return [
...
//   of the whole array.
export function normalizeVec2s(vs, min, max) {
...
export function positionAndTargetToOrthoViewProjMatrix(out, position, target) {
...
export function signedAreaOfTriangle(a, b, c) {
    const ab = tempVec2();
    const ac = tempVec2();
...
export function vec3Reverse(out) {
    const t = out[0];
    out[0] = out[2];
    out[2] = t;
    return out;
}
export function vec4Reverse(out) {
    let t = out[0];
    out[0] = out[3];
    out[3] = t;
    t = out[1];
...
export function vec4RotateLeft(out) {
    let t = out[0];
    out[0] = out[1];
    out[1] = out[2];
    out[2] = out[3];

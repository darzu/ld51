...
export function registerCameraSystems(em) {
    em.registerSystem(null, [CameraDef, TimeDef], function (_, res) {
    ...
    em.registerSystem(null, [CameraDef], function ([], res) {
        ...
        if (prevTarget && newTarget) {
            ...
            const computedRotation = quat.mul(tempQuat(), prevTarget.world.rotation, res.camera.lastRotation);
            const newComputedRotation = quat.mul(tempQuat(), newTarget.world.rotation, res.camera.rotationOffset);
    ...
    em.registerSystem(null, [CameraViewDef, CameraDef, MeDef, CanvasDef], (_, resources) => {
        ...
        if (targetEnt) {
            const computedTranslation = vec3.add(tempVec3(), frame.position, camera.targetPositionError);
            mat4.fromRotationTranslationScale(viewMatrix, frame.rotation, computedTranslation, frame.scale);
            vec3.copy(cameraView.location, computedTranslation);
        }
        const computedCameraRotation = quat.mul(tempQuat(), camera.rotationOffset, camera.rotationError);
        mat4.multiply(viewMatrix, viewMatrix, mat4.fromQuat(mat4.create(), computedCameraRotation));
        const computedCameraTranslation = vec3.add(tempVec3(), camera.positionOffset, camera.cameraPositionError);
        mat4.translate(viewMatrix, viewMatrix, computedCameraTranslation);
        mat4.invert(viewMatrix, viewMatrix);
        const projectionMatrix = mat4.create();
        if (camera.perspectiveMode === "ortho") {
            const ORTHO_SIZE = 10;
            mat4.ortho(projectionMatrix, -ORTHO_SIZE, ORTHO_SIZE, -ORTHO_SIZE, ORTHO_SIZE, -400, 100);
        }
        else {
            mat4.perspective(projectionMatrix, camera.fov, cameraView.aspectRatio, 1, 100000.0 /*view distance*/);
        }

...
let _tempSplinterMesh = createEmptyMesh("splinterEnd");
function removeSplinterEnd(splinterIdx, wood) {
...
function createSplinterEnd(seg, boardMesh, top, W, D) {
    ...
    // TODO(@darzu): perf? probably don't need to normalize, just use same surface ID and provoking vert for all
    const cursor = mat4.fromRotationTranslation(mat4.create(), rot, pos);
    let _splinterMesh = createEmptyMesh("splinterEnd");
    {
...
export function reserveSplinterSpace(wood, maxSplinters) {
    ...
    range(maxSplinters * _vertsPerSplinter).forEach((_) => wood.mesh.pos.push(vec3.create()));
    range(maxSplinters * _trisPerSplinter).forEach((_) => wood.mesh.tri.push(vec3.create()));
    range(maxSplinters * _quadsPerSplinter).forEach((_) => wood.mesh.quad.push(vec4.create()));

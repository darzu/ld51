...
D50_to_D65 as XYZD50ToXYZD65, D65_to_D50 as XYZD65ToXYZD50, Lab_to_LCH as CIELABToLCH, Lab_to_XYZD50 as XYZCIELABToXYZD50, LCH_to_Lab as LCHToCIELAB, XYZD50_to_CIELAB as XYZD50ToCIELAB, } from "./color-lab.js";
...
export function fromHue(h) {
    return clampLCH({ l: 80, c: 132, h });
}
export function FLRGBToLRGB({ flr, flg, flb }) {
...
export function FRGBToRGB({ fr, fg, fb }) {
    // if (fr > 1 || fg > 1 || fb > 1) {
    //     // TODO(@darzu):
...
export function RGBToFRGB({ r, g, b }) {
    return {
...
export function LRGBToFLRGB({ lr, lg, lb }) {
    return {
...
export function toHex(clr) {
    return RGBToHex(toRGB(clr));
}
export function toString(clr) {
    if (isHSL(clr))
...
export function perceptualLum(color) {
    // https://www.w3.org/TR/2008/REC-WCAG20-20081211/#relativeluminancedef
...
export function clampLCH({ l, c, h }) {
    l = clamp(l, 0, 100);
    c = clamp(c, 0, 132);
    h = clamp(h, 0, 360);
    return { l, c, h };
...
export function clampFRGB({ fr, fg, fb }) {
    fr = clamp(fr, 0, 1.0);
    fg = clamp(fg, 0, 1.0);
    fb = clamp(fb, 0, 1.0);
    return { fr, fg, fb };
}
export function clampFLRGB({ flr, flg, flb }) {
    flr = clamp(flr, 0, 1.0);
    flg = clamp(flg, 0, 1.0);
    flb = clamp(flb, 0, 1.0);
    return { flr, flg, flb };
...
export function contrastClamp(clr, ref, min, max, mod) {
...
function colorDistEuclid({ l: l1, a: a1, b: b1 }, { l: l2, a: a2, b: b2 }) {
    // https://en.wikipedia.org/wiki/Color_difference
    // const c1 = { x: a1, y: b1 }
    // const c2 = { x: a2, y: b2 }
...
export function colorDistDeltaE2000(lab1, lab2) {
...
export function colorDistOKLAB(c1, c2) {
    return colorDistEuclid(toOKLAB(c1), toOKLAB(c2));
}
// helpers
export function makeRainbow(numColors) {
    ...
    const rainbow = range(numColors).map((i) => toV3(toFRGB(mkColor(2 * Math.PI * (i / numColors)))));

...
*/
export function registerNarrowPhaseSystems(em) {
    // TODO(@darzu):
}
export function doesSimplexOverlapOrigin(s) {
    if (s.length !== 4)
        return false;
    const tris = [
        [s[0], s[1], s[2]],
...
export function penetrationDepth(s1, s2, simplex, offset = [0, 0, 0]) {
    if (vec3.equals(s1.travel, s2.travel))
        return Infinity;
    ...
    let minD = Infinity;
    let minPerp = [NaN, NaN, NaN];
    let minVs = [];
    let minNotV = [NaN, NaN, NaN];

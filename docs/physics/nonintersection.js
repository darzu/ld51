import { ColliderDef, DefaultLayer } from "./collider.js";
import { EM } from "../entity-manager.js";
import { mat4, quat, vec3 } from "../gl-matrix.js";
import { computeContactData, computeReboundData, PAD, } from "./phys.js";
import { aabbCenter, checkBroadphase, collisionPairs, copyAABB, createAABB, doesOverlapAABB, doesTouchAABB, rayHitDist, resetCollidesWithSet, transformAABB, } from "./broadphase.js";
import { IDENTITY_FRAME, PhysicsParentDef, PositionDef, TransformDef, updateFrameFromPosRotScale, } from "./transform.js";
import { idPair } from "../util.js";
...
export function doesOverlap(a, b) {
    ...
    if (a.parentOId === b.parentOId)
        ...
        return doesOverlapAABB(a.aabb, b.aabb);
}
export function doesTouch(a, b, threshold) {
    if (a.parentOId === b.parentOId)
        ...
        return doesTouchAABB(a.aabb, b.aabb, threshold);
}
// PRECONDITION: assumes world frames are all up to date
export function registerUpdateWorldAABBs(em, s = "") {
    em.registerSystem([PhysicsStateDef, WorldFrameDef, TransformDef], [], (objs, res) => {
        for (let o of objs) {
...
export const PhysicsBroadCollidersDef = EM.defineComponent("_physBColliders", () => {
    return {
...
export function registerPhysicsStateInit(em) {
    ...
    em.registerSystem([ColliderDef, PositionDef], [PhysicsBroadCollidersDef], (objs, { _physBColliders }) => {
        for (let o of objs) {
            if (PhysicsStateDef.isOn(o)) {
            ...
            if (o.collider.shape === "AABB") {
                _phys.colliders.push(mkCollider(o.collider.aabb, o.id, parentId, o.collider.myLayers, o.collider.targetLayers));
            }
            else if (o.collider.shape === "Multi") {
                for (let c of o.collider.children) {
                    if (c.shape !== "AABB")
                        throw `Unimplemented child collider shape: ${c.shape}`;
                    _phys.colliders.push(mkCollider(c.aabb, o.id, parentId, o.collider.myLayers, o.collider.targetLayers));
                }
            }
            else {
        ...
        function mkCollider(selfAABB, oId, parentOId, myLayers, targetLayers) {
            ...
            if (myLayers)
                for (let l of myLayers)
                    my |= 0b1 << l;
            let target = targetLayers ? 0 : 0xffff;
            if (targetLayers)
                for (let l of targetLayers)
                    target |= 0b1 << l;
...
export function registerUpdateInContactSystems(em) {
    em.registerSystem([ColliderDef, PhysicsStateDef, WorldFrameDef], [PhysicsBroadCollidersDef, PhysicsResultsDef], (objs, res) => {
        // build a dict
        ...
        for (let [contactId, lastData] of contactData) {
            ...
            if ((aParent !== lastData.parentOId && a.id !== lastData.parentOId) ||
...
export function registerPhysicsContactSystems(em) {
    ...
    em.registerSystem([ColliderDef, PhysicsStateDef, PositionDef, WorldFrameDef], [PhysicsBroadCollidersDef, PhysicsResultsDef], (objs, res) => {
        // build a dict
        // TODO(@darzu): would be nice if the query could provide this
        _objDict.clear();
        for (let o of objs)
        ...
        const currColliders = objs
            ...
            .reduce((p, n) => [...p, ...n], []);
        const { collidesWith: colliderCollisions, checkRay: collidersCheckRay } = checkBroadphase(currColliders);
        ...
        while (anyMovement && itr < COLLISION_MAX_ITRS) {
            ...
            for (let [aCId, bCId] of colliderPairs) {
                if (bCId < aCId)
                    throw `a,b id pair in wrong order ${bCId} < ${aCId}`;
                ...
                if (a.collider.solid && b.collider.solid) {
                    ...
                    if (aRebound < Infinity)
                        nextObjMovFracs[aOId] = Math.max(nextObjMovFracs[aOId] || 0, aRebound);
                    if (bRebound < Infinity)
                        nextObjMovFracs[bOId] = Math.max(nextObjMovFracs[bOId] || 0, bRebound);
                }
            }

...
export const DbgMeshDef = EM.defineComponent("_physDbgMesh", (colliderId) => {
    return {
...
export function registerPhysicsDebuggerSystem(em) {
    ...
    // add collider meshes
    em.registerSystem([PhysicsStateDef], [PhysicsDbgDef, AssetsDef], (es, res) => {
        for (let e of es) {
            if (!res._physDbgState.colliderMeshes.has(e.id)) {
                for (let c of e._phys.colliders) {
                    ...
                    em.addComponent(dbgE.id, RenderableConstructDef, res.assets.wireCube.proto, res._physDbgState.showAABBs, 1);
                    // colored
    ...
    em.registerSystem([DbgMeshDef, WorldFrameDef, ...LocalFrameDefs], [PhysicsBroadCollidersDef], (es, res) => {
        for (let e of es) {
...
export function setCubePosScaleToAABB(e, aabb) {
    // cube scale 1 means length 2 sides
    for (let i = 0; i < 3; i++)
        e.position[i] = (aabb.min[i] + aabb.max[i]) * 0.5;
    for (let i = 0; i < 3; i++)
        e.scale[i] = (aabb.max[i] - aabb.min[i]) * 0.5;
}

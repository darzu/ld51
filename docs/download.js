...
function setDropHandlersOnElementInternal(el, dragOverCb, dropCb) {
    if (window.__sprigondrag)
        throw `We've already set drop handlers!`;
    window.__sprigondrag = dragOverCb;
    window.__sprigondrop = dropCb;
    el.setAttribute("ondrop", "window.__sprigondrop(event);");
    el.setAttribute("ondragover", "window.__sprigondrag(event);");
}
export function triggerDownload(filename, body) {
    const fakeBtn = document.createElement("a");
    fakeBtn.setAttribute("href", "data:text/plain;charset=utf-8," + encodeURIComponent(body));

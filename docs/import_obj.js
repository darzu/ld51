...
function parseFaceVert(s) {
    // parse v1/t1/n1 into [v1, t1, n1]
    const parts = s.split("/");
    if (parts.length !== 3)
        return `invalid face vertex: ${s}`;
    const nums = parts.map((s) => parseFloat(s));
    return nums;
}
function parseFace(p) {
...
function parseLineVert(s) {
    // parse v1/t1 into [v1, t1]
    const parts = s.split("/");
    if (parts.length !== 2)
        return `invalid line vertex: ${s}`;
    const nums = parts.map((s) => parseFloat(s));
    return nums;
}
...
export function importObj(obj) {
    ...
    function nextMesh() {
        ...
        const m = { pos, tri, quad, colors, lines };
        meshes.push(m);
        // start a new mesh
    ...
    for (let rawL of lns) {
        ...
        if (!kind) {
            ...
            if (inds.length !== 2)
                return `Too many indices in line: ${p.join(" ")}`;
            lines.push(inds);
        }
        else if (kind === "f") {
            // parse face
    ...
    function sortByGreedyDistance(inds) {
...
export function testImporters() {
  ...
  `);
    assert(good2.tri.length === 0 && good2.quad.length === 1, "test expects 2 tris");

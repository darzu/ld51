...
import { AssetsDef, GROUNDSIZE } from "./assets.js";
...
export const { BoatPropsDef, BoatLocalDef, createBoat } = defineNetEntityHelper(EM, {
    name: "boat",
    defaultProps: (loc, speed, wheelSpeed, wheelDir) => {
        return {
...
export const raiseBreakBoat = eventWizard("break-boat", [[BoatLocalDef, PositionDef, RotationDef]], ([boat]) => {
    const res = EM.getResources([AssetsDef, MusicDef]);
    breakBoat(EM, boat, res.assets.boat_broken, res.music);
});
export function registerBoatSystems(em) {
    em.registerSystem([BoatLocalDef, RotationDef, LinearVelocityDef, AuthorityDef], [PhysicsTimerDef, MeDef], (boats, res) => {
        for (let i = 0; i < res.physicsTimer.steps; i++) {
            for (let o of boats) {
                ...
                // rotate velocity
                vec3.rotateY(o.linearVelocity, [o.boatLocal.speed, -0.01, 0], [0, 0, 0], o.boatLocal.wheelDir);
            }
        }
    }, "stepBoats");
    em.registerSystem([BoatLocalDef, AuthorityDef], [PhysicsTimerDef, MeDef, PhysicsResultsDef], (boats, res) => {
        const ms = res.physicsTimer.period * res.physicsTimer.steps;
        for (let o of boats) {
            if (o.authority.pid !== res.me.pid)
                continue;
            // TODO(@darzu): COUNT DOWN FIREZONE
            const hits = res.physicsResults.collidesWith.get(o.boatLocal.fireZoneId);
            const seesPlayer = hits === null || hits === void 0 ? void 0 : hits.some((h) => !!em.findEntity(h, [ShipLocalDef]));
            if (seesPlayer) {
    ...
    em.registerSystem([BoatLocalDef, PositionDef, RotationDef], [PhysicsResultsDef, AssetsDef, MusicDef, MeDef, DetectedEventsDef], (objs, res) => {
...
export function registerBoatSpawnerSystem(em) {
    em.addSingletonComponent(BoatSpawnerDef);
    em.registerSystem(null, [BoatSpawnerDef, PhysicsTimerDef, GroundSystemDef, GameStateDef, MeDef], (_, res) => {
        if (!res.me.host)
            return;
        ...
        if (res.boatSpawner.timerMs < 0) {
            ...
            // ~1 second minimum
            res.boatSpawner.timerIntervalMs = Math.max(1500, res.boatSpawner.timerIntervalMs);
            // console.log("boat ");
            // create boat(s)
            const boatCon = em.addComponent(em.newEntity().id, BoatPropsDef);
            const left = Math.random() < 0.5;
            const z = res.groundSystem.nextScore * 10 + 100;
            boatCon.location = vec3.fromValues(-(Math.random() * 0.5 + 0.5) * GROUNDSIZE, 10, z);

...
import { createHexGrid, hexDist, hexesWithin, hexX, hexZ, HEX_DIRS, xzToHex, hexDirCCW90, hexDirCW90, hexXYZ, } from "../hex.js";
...
import { addSpawner } from "./spawn.js";
...
export const { GroundPropsDef, GroundLocalDef, createGroundNow } = defineNetEntityHelper(EM, {
    name: "ground",
    ...
    build: (g, res) => {
        ...
        em.ensureComponentOn(g, RenderableConstructDef, res.groundMesh.mesh.proto);
        // TODO(@darzu): instead of individual colliders, I should have big, shared AABB colliders
        em.ensureComponentOn(g, ColliderDef, res.groundMesh.mesh.mkAabbCollider(true));
...
export function initRiverTileSystem(em) {
    // init ground mesh
    em.whenResources([AssetsDef, RendererDef]).then((rs) => {
        const mesh = em.addSingletonComponent(GroundMeshDef);
        // create mesh
        const t = mat4.fromRotationTranslationScale(mat4.create(), quat.IDENTITY, [0, -DEPTH, 0], [WIDTH * 0.5, DEPTH, WIDTH * 0.5]);
        const m = cloneMesh(rs.assets.hex.mesh);
        transformMesh(m, t);
        mesh.mesh = gameMeshFromMesh(m, rs.renderer.renderer);
    });
    em.whenResources([MeDef, GroundMeshDef]).then((rs) => {
    ...
    em.registerSystem(null, [MeDef, GroundSystemDef, GroundMeshDef], (_, rs) => {
...
const raiseGroundEvent = eventWizard("raiseGround", [
    [
        GroundPropsDef,
        GroundLocalDef,
        PositionDef,
...
function raiseTile(res, q, r, color, delayMs, durationMs) {
    const endPos = [hexX(q, r, SIZE), Y, hexZ(q, r, SIZE)];
    let g;
...
function raiseNodeTiles(res, n, easeDelayMs, easeMsPer, doSpawn) {
    ...
    for (let qr of hexesWithin(n.q, n.r, w)) {
        ...
        if (!sys.grid.has(q, r)) {
            ...
            const relQRS = [relQR[0], relQR[1], -relQR[0] - relQR[1]];
            const isOnPath = (p) => vec2.exactEquals(qr, [p.q, p.r]) || (p.next ? isOnPath(p.next) : false);
...
const dropTileEvent = eventWizard("dropTile", [[GroundPropsDef, PositionDef, AngularVelocityDef]], ([g], args) => {
...
function dropNodeTiles(sys, n, easeDelayMs, easeMsPer) {
    let nextEaseDelayMs = easeDelayMs;
    const w = Math.floor(n.width / 2);
    let droppedTiles = [];
...
export function registerGroundSystems(em) {
    em.registerSystem(null, [GroundSystemDef, GroundMeshDef, ScoreDef, MeDef, GameStateDef], (_, res) => {
        // host only
        if (!res.me.host)
            return;
        // is our path inited?
        ...
        if (shipQ === sys.lastShipQ && shipR === sys.lastShipR)
            return;
        // check for tiles to reveal
        {
            const firstAheadFn = (p) => !p || p.state === "ahead" ? p : firstAheadFn(p === null || p === void 0 ? void 0 : p.next);
        ...
        let numDropped = 0;
        {
            assert(sys.currentPath.state === "inplay", "The current path should always be inplay");
            const lastInPlayFn = (p) => p.prev && p.prev.state === "inplay"
                ? lastInPlayFn(p.prev)
                : p.state === "inplay"
                    ? p
                    : undefined;
...
//# sourceMappingURL=river-tile.js.map
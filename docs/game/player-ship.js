...
export const { RudderPropsDef, RudderLocalDef, createRudderNow } = defineNetEntityHelper(EM, {
    ...
    build: (rudder, res) => {
        ...
        em.ensureComponentOn(rudder, RenderableConstructDef, res.assets.rudder.mesh);
        ...
        const interactBox = em.newEntity();
        em.ensureComponentOn(interactBox, PhysicsParentDef, rudder.rudderProps.shipId);
        ...
        constructNetTurret(rudder, 0, 0, interactBox, Math.PI, -Math.PI / 8, 1.5, [0, 20, 50]);
...
export const { PlayerShipPropsDef, PlayerShipLocalDef, createPlayerShip } = defineNetEntityHelper(EM, {
    ...
    build: async (s, res) => {
        ...
        if (s.authority.pid === res.me.pid) {
            ...
            const cannonR = createCannon([-6, 3, 5], Math.PI * 0.5, cannonPitch, s.id);
            s.playerShipProps.cannonRId = cannonR.id;
            const cannonL = createCannon([6, 3, 5], Math.PI * 1.5, cannonPitch, s.id);

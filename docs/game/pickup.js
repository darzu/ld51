...
export function registerItemPickupSystem(em) {
    em.registerSystem([PlayerEntDef, MotionDef], [DetectedEventsDef], (players, resources) => {
        for (let player of players) {
            if (player.player.hat === 0 && player.player.interactingWith > 0) {
                ...
                vec3.transformQuat(dropLocation, dropLocation, player.motion.rotation);

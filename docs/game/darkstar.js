...
import { RenderableConstructDef, RenderDataStdDef, } from "../render/renderer-ecs.js";
...
onInit((em) => {
    ...
    em.registerSystem([DarkStarPropsDef, PositionDef, AuthorityDef], [MeDef, GameStateDef], (es, res) => {
        if (res.gameState.state !== GameState.PLAYING) {
            return;
        }
        for (let star of es) {
            if (star.authority.pid !== res.me.pid)
                continue;
            const toCenter = vec3.sub(tempVec3(), star.darkStarProps.orbiting, star.position);
            ...
            */
            const movementDirection = vec3.cross(tempVec3(), toCenter, star.darkStarProps.orbitalAxis);
            vec3.normalize(movementDirection, movementDirection);
            vec3.add(star.position, star.position, vec3.scale(movementDirection, movementDirection, DARKSTAR_SPEED));

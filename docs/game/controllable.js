...
export function registerControllableSystems(em) {
    const steerVel = vec3.create();
    em.registerSystem([ControllableDef, LinearVelocityDef, RotationDef, WorldFrameDef], [InputsDef, MeDef, CanvasDef, TimeDef], (controllables, res) => {
        for (let c of controllables) {
            ...
            if (modes.canYaw)
                quat.rotateY(c.rotation, c.rotation, -res.inputs.mouseMovX * c.controllable.turnSpeed);
        }
    }, "controllableInput");
    em.registerSystem([ControllableDef, CameraFollowDef], [InputsDef, MeDef, CanvasDef], (controllables, res) => {
        for (let c of controllables) {
            if (AuthorityDef.isOn(c) && c.authority.pid !== res.me.pid)
                continue;
            if (c.controllable.requiresPointerLock &&

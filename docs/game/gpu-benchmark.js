...
let benchmarkTexsAndPipes = [];
export function initBenchmark() {
    benchmarkTexsAndPipes = range(20).map((s) => createBenchmarkTexAndPipe((s + 1) * 128, "gpuTest"));
}
export async function runBenchmark() {
    ...
    for (let [tex, pipe] of benchmarkTexsAndPipes) {
        ...
        console.log(`${tex.size[0]}x${tex.size[1]}, ${mb.toFixed(2)}mb, ${afterGPUTest.toFixed(2)}ms`);
        await asyncTimeout(500); // TODO(@darzu): dbg
    }
}
// TODO(@darzu): DBG
export function createBenchmarkTexAndPipe(size, name) {

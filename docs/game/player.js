...
import { CameraDef, CameraFollowDef, setCameraFollowPosition, } from "../camera.js";
...
export function createPlayer(em) {
...
export const LocalPlayerDef = EM.defineComponent("localPlayer", (playerId) => ({
    playerId: playerId || 0,
}));
...
export function registerPlayerSystems(em) {
    em.registerSystem([PlayerPropsDef], [MeDef, AssetsDef], (players, res) => {
        for (let e of players) {
            ...
            if (!RotationDef.isOn(e))
                em.addComponent(e.id, RotationDef, quat.rotateY(quat.create(), quat.IDENTITY, Math.PI));
            if (!LinearVelocityDef.isOn(e))
                em.addComponent(e.id, LinearVelocityDef);
            if (!ColorDef.isOn(e))
                em.addComponent(e.id, ColorDef, [0, 0.2, 0]);
    ...
    em.registerSystem([PlayerDef, WorldFrameDef], [GlobalCursor3dDef], (players, res) => {
    ...
    em.registerSystem([
        ...
        for (let p of players) {
            ...
            const cursor = res.globalCursor3d.cursor();
            if (cursor) {
                if (RenderableDef.isOn(cursor))
                    cursor.renderable.enabled = cheat;
            }
            ...
            if (cheat && inputs.rclick) {
                ...
                for (let xi = 0; xi <= SPREAD; xi++) {
                    for (let yi = 0; yi <= SPREAD; yi++) {
                        ...
                        bullet_axis = vec3.transformQuat(bullet_axis, bullet_axis, p.rotation);
                        const position = vec3.add(vec3.create(), p.world.position, vec3.fromValues(x, y, 0));
            ...
            if (cheat && inputs.keyClicks["r"]) {
                // create our ray
                const r = {
                    org: vec3.add(vec3.create(), p.world.position, vec3.scale(tempVec3(), vec3.multiply(tempVec3(), facingDir, p.world.scale), 3.0)),
                    dir: facingDir,
                };
                playerShootRay(r);
            }
            ...
            function playerShootRay(r) {
                ...
                const color = doesHit ? [0, 1, 0] : [1, 0, 0];
    ...
    em.registerSystem([
        ...
        CameraFollowDef,
        RotationDef,
    ], [PhysicsResultsDef, MeDef], (players, res) => {
        for (let p of players) {
            if (p.authority.pid !== res.me.pid)
                continue;
            if (!p.player.lookingForShip)
                continue;

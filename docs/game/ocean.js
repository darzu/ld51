...
import { unwrapPipeline, unwrapPipeline2, uvMaskTex, uvToNormTex, uvToPosTex, uvToTangTex, UVUNWRAP_MASK, } from "../render/pipelines/xp-uv-unwrap.js";
...
export async function initOcean() {
    ...
    const [uvToPosData, uvToNormData, uvToTangData, sdfData] = await Promise.all(readPromises);
    ...
    const uvToGerstnerDispAndNorm = (outDisp, outNorm, uv) => {
        // TODO(@darzu): impl
        gerstner(outDisp, outNorm, gerstnerWaves, vec2.scale(tempVec2(), uv, 1000), res.time.time * 0.001);
        ...
        const disp = vec3.add(tempVec3(), vec3.scale(tempVec3(), perp, outDisp[0]), vec3.add(tempVec3(), vec3.scale(tempVec3(), norm, outDisp[1]), vec3.scale(tempVec3(), tang, outDisp[2])));
        // outDisp[0] = pos[0] + disp[0] * 0.5;
        // outDisp[1] = pos[1] + disp[1];
        // outDisp[2] = pos[2] + disp[2] * 0.5;
        vec3.add(outDisp, pos, disp);
        const gNorm = vec3.add(tempVec3(), vec3.scale(tempVec3(), perp, outNorm[0]), vec3.add(tempVec3(), vec3.scale(tempVec3(), norm, outNorm[1]), vec3.scale(tempVec3(), tang, outNorm[2])));

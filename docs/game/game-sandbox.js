...
import { jitter, mathMapNEase } from "../math.js";
...
export async function initClothSandbox(em, hosting) {
    ...
    em.registerSystem([ClothConstructDef, ClothLocalDef, WorldFrameDef, ForceDef], [GlobalCursor3dDef, RendererDef, InputsDef, TextDef], (cs, res) => {
        ...
        if (!line)
            line = createLine(vec3.create(), vec3.create(), [0, 1, 0]);
...
export async function initReboundSandbox(em, hosting) {
    const camera = em.addSingletonComponent(CameraDef);
    camera.fov = Math.PI * 0.5;
    let tableId = -1;
    const res = await em.whenResources(AssetsDef, GlobalCursor3dDef, RendererDef, TextDef);
    const g = createGhost(em);
    ...
    let nextSpawnAccu = 0;
    let paused = true;
    em.registerSystem(null, [AssetsDef, TimeDef, InputsDef], (_, res) => {
        // pause/unpause

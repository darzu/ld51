...
function createGrassTile(opts, builder) {
    ...
    const aabbMin = vec3.fromValues(-spacing, 0, -spacing);
    const aabbMax = vec3.fromValues(size + spacing, bladeH * 2, size + spacing);
    m.setUniform(mat4.create(), aabbMin, aabbMax);
    const grassMesh = m.finish();
    return grassMesh;
}
function createGrassTileset(opts, builderBuilder) {
    ...
    function update(target) {
        ...
        const occupied = [];
        const toMoveInds = [];
        const tilePoses = tiles.map(t => getPositionFromTransform(t.transform));
        ...
        for (let i of toMoveInds) {
            ...
            for (let [xi1, zi1] of nearestIs) {
                ...
                const targetPos = [xi1 * opts.tileSize, 0, zi1 * opts.tileSize];
...
function nearestIntegers(target, numInts) {
...
export function initGrassSystem(builderBuilder) {
    if (!RENDER_GRASS) {
        return {
            getGrassPools: () => [],
            update: () => { },

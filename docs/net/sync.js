...
import { AuthorityDef, InboxDef, MeDef, OutboxDef, PeerDef, send, SyncDef, NetStatsDef, RemoteUpdatesDef, } from "./components.js";
...
export function registerSyncSystem(em) {
    em.registerSystem([AuthorityDef, SyncDef], [TimeDef, MeDef], (ents, res) => {
        ...
        if (res.time.step % 3 === 0) {
            ...
            for (let { peer, outbox } of peers) {
                if (res.me.host && !peer.joined)
                    continue;
                const entities = ents.filter((ent) => ent.authority.pid == res.me.pid);
...
export function registerUpdateSystem(em) {
    em.registerSystem([RemoteUpdatesDef], [], (es) => {
        for (const e of es) {
            em.removeComponent(e.id, RemoteUpdatesDef);
        }
    }, "clearRemoteUpdatesMarker");
    em.registerSystem([PeerDef, InboxDef, OutboxDef], [TimeDef, MeDef, NetStatsDef], (peers, res) => {
        for (let { peer: { address }, inbox, outbox, } of peers) {
            ...
            while (updates.length > 0) {
                ...
                let dt = res.time.lastTime - (ts - res.netStats.skewEstimate[address]);

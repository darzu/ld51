...
import { PeerDef, MeDef, InboxDef, OutboxDef, JoinDef, NetworkReadyDef, EventsToNetworkDef, HostDef, send, } from "./components.js";
import { MessageType, MAX_MESSAGE_SIZE } from "./message.js";
import { TimeDef } from "../time.js";
const JOIN_RETRANSMIT = 100;
function registerConnectToServer(em) {
    em.registerSystem([PeerDef], [JoinDef, NetworkReadyDef, EventsToNetworkDef, TimeDef], (peers, { join, eventsToNetwork, time }) => {
        switch (join.state) {
            case "start":
...
function registerHandleJoin(em) {
    ...
    em.registerSystem([PeerDef, InboxDef, OutboxDef], [MeDef], handleJoin, "handleJoin");
}
function registerHandleJoinResponse(em) {
    ...
    em.registerSystem([PeerDef, InboxDef, OutboxDef], [EventsToNetworkDef], handleJoinResponse, "handleJoinResponse");
}
export function registerJoinSystems(em) {

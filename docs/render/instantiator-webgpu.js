...
export function createCyResources(cy, shaders, device) {
    ...
    const cyNameToBufferUsage = {};
    // all buffers are updatable via queue
    // TODO(@darzu): option for some to opt out? for perf?
    [...cy.kindToPtrs.array, ...cy.kindToPtrs.singleton].forEach((r) => (cyNameToBufferUsage[r.name] |= GPUBufferUsage.COPY_DST));
    // all singleton buffers are probably used as uniforms
    cy.kindToPtrs.singleton.forEach((p) => (cyNameToBufferUsage[p.name] |= GPUBufferUsage.UNIFORM));
    // all pipeline global resources are storage or uniform
    // TODO(@darzu): be more precise?
    [...cy.kindToPtrs.compPipeline, ...cy.kindToPtrs.renderPipeline].forEach((p) => p.globals.forEach((r) => {
        if (isResourcePtr(r)) {
    ...
    cy.kindToPtrs.array.forEach((b) => {
        if (b.forceUsage)
            cyNameToBufferUsage[b.name] = b.forceUsage;
    });
    // determine texture usages
    const cyNameToTextureUsage = {};
    ...
    [...cy.kindToPtrs.renderPipeline, ...cy.kindToPtrs.compPipeline].forEach((p) => {
        p.globals.forEach((r) => {
            if (isResourcePtr(r)) {
                // nothing?
            }
    ...
    cy.kindToPtrs.meshPool.forEach((r) => {
        ...
        assert(verts && unis && triInds && lineInds, `Missing buffer for mesh pool ${r.name}`);
    ...
    for (let p of [
        ...
        function mkBindGroupLayout(ptrs, dynamic) {
        ...
        function globalToWgslDefs(r, plurality) {
            if (r.ptr.kind === "singleton" || r.ptr.kind === "array") {
                const structStr = `struct ${capitalize(r.ptr.name)} {\n` +
                ...
                if (plurality === "one") {
                    ...
                    const fixedSize = typeof r.ptr.init === "number" ? `, ${r.ptr.init}` : "";
                    return (structStr +
        ...
        if (isRenderPipelinePtr(p)) {
            ...
            const targets = output.map((o) => {
                return {
                    format: o.ptr.format,
                };
            });
            let depthStencilOpts = undefined;
            if (p.depthStencil) {
            ...
            if (p.meshOpt.stepMode === "per-instance") {
                ...
                const idxBuffer = kindToNameToRes.idxBuffer[p.meshOpt.pool.triIndsPtr.name];
    ...
    if (VERBOSE_LOG)
        console.log(`createCyResources took: ${(performance.now() - start).toFixed(1)}ms`);
    return {
        kindToNameToRes,
    };
}
function normalizeColorAttachments(atts) {
    return atts.map((a) => {
        if (isResourcePtr(a)) {
            return {
                ptr: a,
...
export function bundleRenderPipelines(device, resources, renderPipelines, meshHandleIds) {
    ...
    for (let p of renderPipelines) {
        ...
        const colorFormats = p.output.map((o) => {
            return o.ptr.format;
        });
        ...
        if (p.bindGroupLayouts.length) {
            ...
            const resBindGroup = mkBindGroup(device, resources, resBindGroupLayout, globalUsages, "many");
        ...
        if (p.ptr.meshOpt.stepMode === "per-instance") {
            ...
            const uniBG = mkBindGroup(device, resources, uniBGLayout, [uniUsage], "one");
...
function mkBindGroupEntry(device, resources, idx, r, bufPlurality) {
    const kindToNameToRes = resources.kindToNameToRes;
    if (r.ptr.kind === "singleton" || r.ptr.kind === "array") {
        const buf = r.ptr.kind === "singleton"
...
export function mkBindGroup(device, resources, layout, ptrs, 
...
export function startBundleRenderer(context, commandEncoder, resources) {
    ...
    function render(p, bundle) {
        ...
        if (p.ptr.depthStencil) {
            ...
            const depthTex = resources.kindToNameToRes.depthTexture[p.ptr.depthStencil.name];
...
export function doCompute(device, resources, commandEncoder, pipeline) {
    // if (!querySet) {
...
export function onCanvasResizeAll(device, context, resources, canvasSize) {

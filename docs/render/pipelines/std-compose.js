...
export function createGridComposePipelines(grid) {
    ...
    for (let ri = 0; ri < grid.length; ri++) {
        for (let ci = 0; ci < grid[ri].length; ci++) {
            ...
            pipes.push(createRenderTextureToQuad(`composeViews_${ci}x${ri}`, tex, canvasTexturePtr, xMin, xMax, yMin, yMax, false).pipeline);
        }
    }
    return pipes;
}

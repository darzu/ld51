...
import { uvPosBorderMask } from "./xp-uv-unwrap.js";
...
export const jfaPreOutlinePipe = createRenderTextureToQuad("jfaPreOutlinePipe", uvPosBorderMask, jfaInputTex, -1, 1, -1, 1, false, 
// VORONOI
() => `
    rand_seed = uv;
    if (rand() < 0.003) {
...
const copy1to0 = createRenderTextureToQuad("jfaCopy", jfaTexs[1], jfaTexs[0], -1, 1, -1, 1, false, () => `
return inPx.xy;
`).pipeline;
export const jfaPipelines = range(maxStep + 1)
    ...
    if (VISUALIZE_JFA)
        return [pipeline, copy1to0];
    else
        return [pipeline];
})
    .reduce((p, n) => [...p, ...n], []);
// TODO(@darzu): this probably isn't needed any more
export const jfaToSdfPipe = createRenderTextureToQuad("jfaToSdf", jfaResultTex, sdfTex, -1, 1, -1, 1, false, () => `
    let nearestUV = textureLoad(inTex, xy, 0).xy;
    let dist = length(uv - nearestUV);
    return dist;
  `).pipeline;
export const sdfBrightPipe = createRenderTextureToQuad("sdfBright", sdfTex, sdfBrightTex, -1, 1, -1, 1, false, () => `
    return inPx * 4.0;
  `).pipeline;
// TODO(@darzu): IMPLG
...
// TODO(@darzu): this probably isn't needed any more
export const sdfToRingsPipe = createRenderTextureToQuad("sdfToRings", sdfTex, ringsTex, -1, 1, -1, 1, false, () => `
  ...
  return vec4(0.0);
  `).pipeline;
//# sourceMappingURL=xp-jump-flood.js.map
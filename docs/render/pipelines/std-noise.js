...
*/
const noiseSizes = [2, 4, 8, 16, 32, 64, 128, 256, 512];
export const whiteNoiseTexs = noiseSizes.map((s) => CY.createTexture(`whiteNoise${s}Tex`, {
    size: [s, s],
    format: "r32float",
}));
...
export const vecNoiseTexs = noiseSizes.map((s) => CY.createTexture(`vecNoise${s}Tex`, {
    size: [s, s],
    format: "rg32float",
}));
...
function createOctaveNoisePipe(frequencies, persistence) {
    // TODO(@darzu): make color channel assignments a parameter?
    const smooth = true; // TODO(@darzu): parameter
    const gradNoise = true;
    const texType = gradNoise
        ? `vecNoise`
        : "whiteNoise";
    assert(frequencies.every((f) => Number.isInteger(f)), "freqs must be int");
    assert(frequencies[frequencies.length - 1] < noiseSizes.length + 1, "freq range");
    ...
    return CY.createRenderPipeline(name + "Pipe", {
        globals: [
            ...
            ...(texType === `whiteNoise` ? whiteNoiseTexs : vecNoiseTexs).map((t) => t),
        ],

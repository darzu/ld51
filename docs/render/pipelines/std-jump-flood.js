...
export function createJfaPipelines(maskTex, maskMode) {
    let size = 512;
    const voronoiTexFmt = {
        size: [size, size],
        format: "rg16float",
  ...
  `;
    ...
    const extractUvMaskPipe = createRenderTextureToQuad(namePrefix + "UvMaskPipe", maskTex, uvMaskTex, -1, 1, -1, 1, false, 
    // TODO(@darzu): output max distance?
    extractUvMaskShader).pipeline;
    ...
    const extractSdfPipe = createRenderTextureToQuad(namePrefix + "JfaToSdf", voronoiResultTex, sdfTex, -1, 1, -1, 1, false, 
    // TODO(@darzu): output max distance?
    () => `

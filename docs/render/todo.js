...
I want the system to handle the agreement problems
  i.e. buffer usage and stage flags, binding and layout indices,
    location indices
*/
import { createCyOne, createCyMany } from "./data.js";
if (false) {
    const device = null;
    ...
    //   rope_shader()
    // );
    let SceneStruct = null;
    let RopePointStruct = null;
    let RopeStickStruct = null;
    ...
    // TRI
    let ModelUniStruct = null;
    let MyDispTexDesc = null;
    let VertexStruct = null;
    let ParticleVertStruct = null;
...
function createCyPipeline(d, a, ...s) {
    return {
        dispatch: (a) => a,
    };
}
...
*/
export function createCyCompPipeline(device) {
